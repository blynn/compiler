...
start of adventures in the browser. A fuller account:
  ...
  * For the REPL, we add a `scratchpad` buffer where the interpreter writes
  new bytecode. For defintions, we write the number of new symbols and call
  ...
  the index of a previous definition. This complexity arises because GC could
  occur while compiling new definitions.
