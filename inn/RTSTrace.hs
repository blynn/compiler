...
SYM x y = "fputs(syms[num(1)], stderr); putc('\\n', stderr); sp[2] = arg(2); sp += 2;"
|]
...
encSym symIdx s t = do
  num <- Code <$> memget (Code $ comEnum "NUM", Code symIdx)
  hx <- Code <$> memget (Code $ comEnum "SYM", num)
  hy <- enc t
  Code <$> memget (hx, hy)

asm count combs = foldM
  (\symtab (n, (s, t)) -> (flip (insert s) symtab) <$> encSym n s t)
  Tip $ zip [count..] combs

codegenLocal ((bigmap, count), (hp, f)) (name, neat) =
  ((insert name localmap bigmap, count + length combs), (hp', f . (mem++)))
  where
  combs = optiComb $ toAscList $ snd <$> typedAsts neat
  (symtab, (_, (hp', memF))) = runState (asm count combs) (Tip, (hp, id))
...
codegen ffis mods = (bigmap', mem) where
  ((bigmap, _), (_, memF)) = foldl codegenLocal ((Tip, 0), (128, id)) $ toAscList mods
...
compileWith topSize libc opts mods = do
  let
    ...
    genSymArray = ("static const char* syms[] = {\n"++)
      . foldr (.) id [ (show s++) . (",\n"++) | (modName, neat) <- toAscList mods, s <- keys $ typedAsts neat ]
      . ("};\n"++)
