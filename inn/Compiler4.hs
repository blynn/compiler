...
fixFixity precs t = case t of
  E _ -> pure t
  V _ -> pure t
  A (E (Basic "{+")) ch -> infixer precs =<< go ch
  A x y -> A <$> go x <*> go y
  L s b -> L s <$> go b
  ...
  where
  go = fixFixity precs
  pgo = pure . patFixFixity precs

infixer precs (A (A (A s x) y) t) = go seed t where
  ...
  rebase s z = \case
    A (A (V s') x) y -> let
      ...
      in extendChain precs stay down s s'
    x -> A (A (V s) x) z
...
patFixer precs (PatCon f [a, b]:rest) = unprotectAll $ foldr rebase seed rest where
...
extendChain precs stay down s s' =
  if prec <= prec'
    then if prec == prec'
      then if assoc == assoc'
        then case assoc of
  ...
  where
  (prec, assoc) = maybe (5, LAssoc) id $ findCon precs s
  (prec', assoc') = maybe (5, LAssoc) id $ findCon precs s'

secondM f (a, b) = (a,) <$> f b
patternCompile precs dcs t = optiApp $ resolveFieldBinds dcs $ evalState (go $ either error id $ fixFixity precs t) 0 where
  go t = case t of
    E _ -> pure t
    V _ -> pure t
...
inferTypeclasses tycl typeOfMethod typed precs dcs linker ienv = concat <$> mapM perClass (toAscList ienv) where
      ...
      perInstance (Instance ty name ps idefs) = do
        let
          ...
          perMethod s = do
            ...
            expr <- snd <$> linker (patternCompile precs dcs rawExpr)
...
neatNew = foldr (\(a, b) -> addAdt a b []) neatEmpty { typedAsts = prims } primAdts
...
inferModule tab acc name = case mlookup name acc of
  Nothing -> do
    let
      Neat rawIenv rawDefs typed adtTab ffis ffes imps precTab = tab ! name
      ...
      precs = singleton ":" (5, RAssoc) : precTab : map (opFixity . (tab !)) imps
    ...
    depdefs <- mapM (\(s, t) -> (s,) <$> linker (patternCompile precs dcs t)) defs
    (qs, lambF) <- inferDefs tycl depdefs (fromList typed)
    mets <- inferTypeclasses tycl typeOfMethod qs precs dcs linker ienv
