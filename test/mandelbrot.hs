...
mand p = null . dropWhile (\z -> norm z < 4*prec) . take 30 . iterate (sqAdd p) $ (0, 0);

x80 = take 80 $ iterate (1+) 0;
y24 = take 24 $ iterate (1+) 0;

str = foldr ($) "" $ map (\y -> (map (\x -> ife (mand (616*x - 2*prec, 1502*y - 18022)) '*' ' ') x80 ++) . ("\n" ++)) y24;
...
main = mapM_ (\y -> (mapM_ (\x -> putChar $ ife (mand (616*x - 2*prec, 1502*y - 18022)) '*' ' ') x80) >> putChar '\n') y24;
